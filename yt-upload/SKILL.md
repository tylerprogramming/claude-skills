...
python3 ~/.claude/skills/yt-upload/yt.py upload \
  ...
  --tags-file ~/youtube/claude-design/tags.txt \
  --thumbnail ~/youtube/claude-design/thumbnail-final.jpg \
  --category 28 \
  --privacy private \
...
1. Pull the title from `~/youtube/<slug>/titles.md` (recommended title).
2. Strip the H1 and frontmatter from `~/youtube/<slug>/description.md` and write to a temp file.
...
6. Save the video_id to `~/youtube/<slug>/youtube_video_id.txt` so future updates can find it.
...
4. Append a note to `~/youtube/<slug>/fallbacks.md` under "Thumbnail change log" with date + reason.

### Edit a title or description after publish

1. Edit `~/youtube/<slug>/description.md` or `titles.md` first (single source of truth).
...
- **Save the video_id** after upload so future edits can find it. Recommend `~/youtube/<slug>/youtube_video_id.txt`.
