#!/usr/bin/env python3
"""Resize images to social media aspect ratios using Kie.ai remix mode."""
...
# Social media presets mapped to Kie.ai aspect ratios
SOCIAL_PRESETS = {
    "instagram_post": {"ratio": "1:1", "label": "Instagram Post (1:1)"},
    "instagram_portrait": {"ratio": "4:5", "label": "Instagram Portrait (4:5)"},
    "instagram_story": {"ratio": "9:16", "label": "Instagram Story / Reels (9:16)"},
    "youtube_thumbnail": {"ratio": "16:9", "label": "YouTube Thumbnail (16:9)"},
    "linkedin_post": {"ratio": "3:2", "label": "LinkedIn Post (3:2)"},
    "twitter_x_post": {"ratio": "16:9", "label": "Twitter / X Post (16:9)"},
    "facebook_post": {"ratio": "3:2", "label": "Facebook Post (3:2)"},
    "pinterest_pin": {"ratio": "2:3", "label": "Pinterest Pin (2:3)"},
    "ultrawide": {"ratio": "21:9", "label": "Ultrawide Banner (21:9)"},
}

# All aspect ratios available on Nano Banana 2
ALL_RATIOS = ["1:1", "1:4", "1:8", "2:3", "3:2", "3:4", "4:1", "4:3", "4:5", "5:4", "8:1", "9:16", "16:9", "21:9", "auto"]
...
def create_task(prompt, api_key, aspect_ratio, image_url, resolution="2K", output_format="png"):
    payload = {
        "model": MODEL_ID,
        "input": {
            "prompt": prompt,
    ...
    print(f"  Creating resize task ({aspect_ratio})...")
...
def main():
    parser = argparse.ArgumentParser(description="Resize images to social media aspect ratios via Kie.ai")
    parser.add_argument("source", help="Image file or folder containing images")
    parser.add_argument("--ratios", nargs="+", required=True,
                        help="Aspect ratios to generate (e.g. 1:1 4:5 9:16 16:9)")
    ...
    parser.add_argument("--output", default=None, help="Output folder (default: ~/images/resized/)")
    parser.add_argument("--prompt", default=None,
                        help="Custom prompt (default: auto-generated preservation prompt)")
    ...
    for r in args.ratios:
        if r not in ALL_RATIOS:
            print(f"Error: Invalid ratio '{r}'. Available: {', '.join(ALL_RATIOS)}")
            sys.exit(1)

    output = Path(args.output).expanduser().resolve() if args.output else Path.home() / "images" / "resized"
    output.mkdir(parents=True, exist_ok=True)

    api_key = get_api_key()

    default_prompt = (
        "Recreate this exact image as faithfully as possible. Preserve the subject, likeness, "
        "colors, lighting, style, and all visual details. Adjust the composition naturally to "
        "fit the new aspect ratio without distorting, stretching, or skewing. Extend or adjust "
        "the background as needed while keeping the subject intact and centered."
    )
    prompt = args.prompt or default_prompt

    total_tasks = len(images) * len(args.ratios)
    est_cost = total_tasks * (0.12 if args.resolution == "4K" else 0.09)

    print(f"{'=' * 50}")
    print(f"Images: {len(images)}")
    print(f"Ratios: {', '.join(args.ratios)}")
    print(f"Resolution: {args.resolution}")
    print(f"Format: {args.format}")
    print(f"Total generations: {total_tasks}")
    ...
    for img_path in images:
        ...
        # Create subfolder per source image
        img_output_dir = output / img_path.stem
        ...
        for ratio in args.ratios:
            ...
            if not task_id:
                print(f"  Failed to create task for {ratio}")
                continue

            urls = poll_task(task_id, api_key)
            if not urls:
                print(f"  Failed to generate {ratio}")
                continue

            for j, url in enumerate(urls):
                suffix = f"_{j + 1}" if len(urls) > 1 else ""
                filename = f"{img_path.stem}_{ratio_label}{suffix}.{args.format}"
                download_image(url, img_output_dir / filename)
                success_count += 1

    print(f"\n{'=' * 50}")
    print(f"Done! Generated {success_count}/{total_tasks} resized images.")
    print(f"Output: {output}")
