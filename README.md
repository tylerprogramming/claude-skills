...
| **YouTube** | `/youtube` | Upload videos to YouTube and manage existing videos via the YouTube Data API v3 (titles, descriptions, tags, thumbnails, scheduling, comments) |
...
2. **`/youtube`** — uses the YouTube Data API v3:
   ...
   - First run: `python ~/.claude/skills/youtube/yt.py auth` to walk the OAuth flow. This writes `youtube/token.json` (gitignored).
...
4. **`/analytics`** — needs its own OAuth token at `analytics/yt_token.json` (gitignored).
