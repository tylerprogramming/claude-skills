...
2. Run the script:
   ...
   ```
   Options: `--slug <name>` to name the folder, `--batch <file>` to run many prompts (separated by a line of `---`), `--timeout <sec>` per model, `--image <path>` for design-to-code (lets each model view a local screenshot via the Read tool), `--rebuild <dir>` to re-skin an existing run's `compare.html` / `results.md` without re-calling the models, `--no-open` to skip auto-opening the viewer.
