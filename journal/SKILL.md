---
name: journal
description: Daily journal and standup logger. Asks what you did, what you're doing, and any blockers, then saves entries to ~/content/journal/. Can also generate and email weekly summaries. Triggers on: journal, standup, daily log, what did I do, weekly summary.
...
All journal entries are stored in `~/content/journal/` as text files named by date: `YYYY-MM-DD.txt`
...
After collecting answers, save the entry to `~/content/journal/YYYY-MM-DD.txt` using today's date. Format:
...
Read all entries from the past 7 days in `~/content/journal/` and generate a summary:
...
## Rules
- Always use `~/content/journal/` for storage — never save journal files elsewhere
